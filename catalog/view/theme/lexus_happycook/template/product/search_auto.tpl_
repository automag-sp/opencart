...
//--></script> 
  ...
  <?php } else if($vendor) { ?>
	<h2><?php echo "$vendor, $model, $year, $mod"; ?></h2>
	<table class="result-avto">
	  <tr>
	  	<td>
		  	<?php if($car['pcd'] || $car['dia'] || $car['nut']) { ?><h4>Общие параметры дисков</h4><?php } ?>
		  	<?php if($car['pcd']) { ?><div><b><?php echo $text_pcd; ?>:</b>&nbsp;<?php echo $car['pcd']; ?></div><?php } ?>
		  	<?php if($car['dia']) { ?><div><b><?php echo $text_dia; ?>:</b>&nbsp;<?php echo $car['dia'] . $text_mm; ?></div><?php } ?>
		  	<?php if($car['nut']) { ?><div><b><?php echo $text_nut; ?>:</b>&nbsp;<?php echo $car['nut']; ?></div><?php } ?>
	  	</td>
	  </tr>
	  <tr>
	  	<td>
	  		<h3><?php echo $text_recom_discs; ?></h3>
	  		<?php if(isset($data['wheels'])) { ?>
	  			<?php foreach($data['wheels'] as $groups => $items) { ?>
					<h4><?php echo $groups; ?></h4>
					<?php foreach($items as $item) { ?>
	  				<div><a target="_blank" href="<?php echo($item['href']); ?>"><?php echo($item['name']); ?></a><?php echo $item['pos'] ? '<small>' . $item['pos'] . '</small>' : ''; ?></div>
	  				<?php } ?>
	  			<?php } ?>
	  		<?php } else { ?>
	  	...
	  	<td>
	  		<h3><?php echo $text_recom_tires; ?></h3>
	  		<?php if(isset($data['tires'])) { ?>
	  			<?php foreach($data['tires'] as $groups => $items) { ?>
					<h4><?php echo $groups; ?></h4>
					<?php foreach($items as $item) { ?>
	  				<div><a target="_blank" href="<?php echo($item['href']); ?>"><?php echo($item['name']); ?></a><?php echo $item['pos'] ? '<small>' . $item['pos'] . '</small>' : ''; ?></div>
	  				<?php } ?>
