...
?>
        ...
        <td valign="top" style="padding-left: 10px">
            <img src="http://img.saleepc.ru/hyundai/<?=$saleepc['model']['CatFolder']?>/<?=$saleepc['subgroup']['subgroup_id']?>.png" usemap="#Navigation"/>
            <p><map name="Navigation">
<?
            foreach ($saleepc['img_map'] as $itm){
                if ($itm['typ'] ==5) {
                    echo '<area shape="rect" coords="'.($itm['x']).', '.($itm['y']).', '.($itm['x1']).', '.($itm['y1']).'" href="/index.php?route=saleepc/hyundai/parts&model='.$saleepc['model']['CatFolder'].'&subgroup='.$itm['itm_id'].'11'.$q.'" title="'.$itm['itm_id'].'">';
                } else {
