...
<?
	...
	foreach ((array)$models as $region => $result){
		...
		foreach ($result as $model){
			echo '<tr onclick="window.location.href=\'/index.php?route=saleepc/seat/years&model='.$model['id'].'\'"><td>'.$model['model'].'</td><td>'.$model['modelname'].'</td><td>'.$model['yfrom'].' - '.$model['yto'].'</td><td><a href="/index.php?route=saleepc/seat/years&model='.$model['id'].'" title="'.$model['modelname'].'"> >>> </a></td></tr>';
		}
		echo '</table>';} else {
