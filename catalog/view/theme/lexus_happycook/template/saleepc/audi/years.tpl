...
<?
	foreach ($saleepc['years'] as $year){
		echo '<tr onclick="window.location.href=\'/index.php?route=saleepc/audi/groups&model='.$year['id'].'\'"><td>'.$year['yfrom'].'</td><td>'.($year['modelname']==''?'':'<pre style="margin: 0;">'.$year['modelname']).'</pre></td><td><a href="/index.php?route=saleepc/audi/groups&model='.$year['id'].'" title="'.$saleepc['model']['modelname'].'"> >>> </a></td></tr>';
	}
?>
	</table>
