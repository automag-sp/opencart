...
function show(imageName)
{
    ...
    */
    node.innerHTML = '<img src="http://img.autosovet.ru/vag12/VW/'+imageName.substr(6, 3)+'/'+imageName.substr(6, 3)+imageName.substr(1, 5)+imageName.substr(0, 1)+'.gif" width=450>';
}
</script>
...
<?			
	foreach ($saleepc['subgroups'] as $subgroup){
	    foreach ($subgroup['descrs'] as $row){
            echo '<tr onclick="show(\''.$subgroup['img'].'\')" onDblClick="document.location=\'/index.php?route=saleepc/vw/parts&model='.$saleepc['year']['id'].'&subgroup='.$row['id'].'\'" style="cursor: pointer;"><td nowrap>'.substr($subgroup['hgug'], 0, 1).' - '.substr($subgroup['hgug'], 1, 2).'</td><td>';
            ////////////////////////////////////////////
                $texts = $row['tsben'];
