...
class ControllerCheckoutCart extends Controller {
	...
	public function addR() {
		...
		$NOMER = mb_strtoupper(preg_replace('/[^a-z0-9]/ium', '', $this->request->post['nomer']));
		$DAYS = $this->request->post['days'];
		
        $ext_products = @file_get_contents('http://voshod-avto.ru.public.api.abcp.ru/search/articles/?userlogin=automag24@automag-sp.ru&userpsw=13d5540298788ca341cf3ed24632b79f&number='.$this->request->post['nomer'].'&brand='.$this->request->post['brand'], FALSE, stream_context_create(array('http'=>array('ignore_errors' => TRUE))));		    
        $ext_products = json_decode($ext_products, true);
   		if (!isset($ext_products['errorCode'])){
   		    ...
   		    foreach ($ext_products as $product){
   		        ...
   		        if ($product['numberFix'] <> $NOMER) continue;
   		        if (ceil((int)(preg_replace('/[^a-z0-9]/ium', '',$product['deliveryPeriod']))/24) <> $DAYS) continue;
   		        if ($pp['price']>$product['price']) $pp = $product;
   		    }
   		    
            $product2 = array();
            $product2['model'] = $pp['number'];
            $product2['sku'] = $pp['numberFix'];
            $product['manufacturer'] = $BRAND;
            $product2['name'] = $pp['description'];
            $product2['price'] = $pp['price']*1.35;
            $product2['quantity'] = 0;
            $product2['weight'] = 0;
            $product2['category'] = 148;
            //$this->model_catalog_priceloader->putNewProduct($product);
            $product_id = $this->putNewProduct($product2);
	...
	public function addR2() {
		...
		$NOMER = mb_strtoupper(preg_replace('/[^a-z0-9]/ium', '', $this->request->get['nomer']));
		$DAYS = $this->request->get['days'];
		
        $ext_products = @file_get_contents('http://voshod-avto.ru.public.api.abcp.ru/search/articles/?userlogin=automag24@automag-sp.ru&userpsw=13d5540298788ca341cf3ed24632b79f&number='.$this->request->get['nomer'].'&brand='.$this->request->get['brand'], FALSE, stream_context_create(array('http'=>array('ignore_errors' => TRUE))));		    
        $ext_products = json_decode($ext_products, true);
...
private function putNewProduct($product) {
        //$query = $this->db->query("SELECT product_id FROM ". DB_PREFIX ."product WHERE sku = '".mb_strtoupper(preg_replace('/[^a-z0-9]/ium', '', $product['sku']))."' AND manufacturer_id = ".(int)$product['manufacturer']);
        //if ($query->rows) {
        if (false) {
            return $query->row->product_id;
        } else {
            $this->db->query("INSERT INTO " . DB_PREFIX . "product SET 
                `model` = '" . mb_strtoupper(preg_replace('/[^a-z0-9]/ium', '', $product['model'])) . "', 
