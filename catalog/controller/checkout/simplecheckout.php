...
class ControllerCheckoutSimpleCheckout extends Controller {
    ...
    private function order() {
        ...
        if (empty($customer['email'])) {
            ...
            $email = !empty($email) ? $email : 'anon' . (isset($customer['telephone']) ? $customer['telephone'] : mktime()) . '@automag-sp.ru';
            $customer['email'] = $email;
        }
        if (($customer['email'] == 'info@automag-sp.ru')
            && !empty($customer['telephone'])
        ) {
            $customer['email'] = 'anon' . preg_replace('/\D/', '', $customer['telephone']) . '@automag-sp.ru';
        }
        if(empty($customer['firstname'])){
            list($customer['firstname'],$customer['lastname']) = explode('@',$customer['email']);
        }
//        trigger_error($customer['email']);
