        ...
        if (items.length == 1) {
            ...
            jQuery.colorbox({
                'html':function () {
                    ...
                    var html = '<h2><span>' + name + '</span></h2>' + '<table class="diffs">';
                    ...
                    for (var idx = 0; idx < count; idx++) {
                        citem = jQuery(notes[idx]);
                        noteItems = citem.find('.item');
                        noteItemOptions[idx] = new Array();
                        countNoteItems = noteItems.length;
                        for (iNoteItems = 0; iNoteItems < countNoteItems; iNoteItems++) {
                            cNoteItem = jQuery(noteItems[iNoteItems]);
                            cNoteItemName = cNoteItem.find('.name').html();
                            if (columns.indexOf(cNoteItemName) == -1) {
                                columns[columns.length] = cNoteItemName;
                            }
                            noteItemOptions[idx][cNoteItemName] = cNoteItem.find('.value').html()
                        }
                    }

                    html += '<thead><th >OEM</th><th>Name</th>';
                    var columnCount = columns.length;
                    for (idx = 0; idx < columnCount; idx++) {
                        html += '<th>' + columns[idx] + '</th>';
                    }
                    html += '</thead><tbody>';
                    ...
                    for (idx = 0; idx < count; idx++) {
                        ...
                        url += (url.indexOf('?') >= 0 ? '&' : '?') + 'format=raw';
                        html += '<tr><td><a href="#" onclick="jQuery.colorbox({\'href\': \'' + url + '\',\'opacity\': 0.3, \'innerWidth\' : \'1000px\',\'maxHeight\' : \'98%\'}); return false;">' + jQuery(oems[idx]).text() + '</a></td>' +
                            '<td>' + jQuery(items[idx]).text() + '</td>';
                        for (iNoteItems = 0; iNoteItems < columnCount; iNoteItems++) {
                            html += '<td>' + noteItemOptions[idx][columns[iNoteItems]] + '</td>'
                        }
                    }
