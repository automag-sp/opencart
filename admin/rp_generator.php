			...
			foreach ($query->rows as $product) {
				...
				if ($rp_number < $nomax)
					{						
							...
							inner join (SELECT product_id, MATCH(name, description) AGAINST ('". strip_tags(trim($db->escape(htmlspecialchars_decode(str_replace($bef, $aft,$product['pname']))))) . ' ' . strip_tags(trim(mysql_real_escape_string(htmlspecialchars_decode(str_replace($bef, $aft,$product['pdescription']))))) ."') as rlv FROM " . DB_PREFIX . "product_description) as rel on rel.product_id = p.product_id  
